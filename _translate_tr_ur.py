#!/usr/bin/env python3
"""
Translate sub-page ar.json -> tr.json and ur.json using Google Translate with batching.
"""
...
def process_page(prefix, folder, lang_code, target_code):
    ar_path = f"{folder}/ar.json"
    with open(ar_path, 'r', encoding='utf-8') as f:
        ar_data = json.load(f)

    out_path = f"{folder}/{lang_code}.json"
    existing = {}
    ...
    if not to_translate:
        print(f"[{prefix}] {lang_code}: already up to date")
    ...
    print(f"[{prefix}] {lang_code}: translating {len(to_translate)} keys...")
    translator = GoogleTranslator(source='ar', target=target_code)
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(new_data, f, ensure_ascii=False, indent=2)
    print(f"[{prefix}] {lang_code}: saved {len(new_data)} keys")

for prefix, folder in pages:
    process_page(prefix, folder, 'tr', 'tr')
    time.sleep(1)

for prefix, folder in pages:
    process_page(prefix, folder, 'ur', 'ur')
    time.sleep(1)

print("Turkish & Urdu translation complete!")
